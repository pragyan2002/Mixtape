...
export function NavBar({ page, onChange, trackCount, downloadCount }: NavBarProps) {
  return (
    <nav className="glass relative z-10 flex items-center gap-1 px-4 py-2 shrink-0">
      <div className="flex items-center gap-2 mr-6">
        <span className="text-primary text-lg">🎵</span>
        <span className="font-bold text-text tracking-tight">Mixtape</span>
      </div>
      {tabs.map(({ id, label, Icon }) => {
        ...
        return (
          ...
          >
            ...
            {label}
            {badge !== null && (
              <span className="ml-1 text-xs bg-surface-2 text-text rounded-full px-1.5 py-0.5 leading-none">
                {badge}
              </span>
            )}
          </button>
